...
## v6：反向局部压缩诊断（2026-09-23）

[完整结果与原始记录](outputs/prefill_compute_goal/failure_causality_v6/README.md)：36/36单元，3个已知失败样本 +3个正确对照，属于事后机制诊断。

- 之前“多数chunk压缩、只保留仓库证据chunk”救回2/3道错题。
- 现在反过来“多数chunk不压、只压仓库证据chunk”，3/3道都保持答对；只压仓库与箱数两处证据也是3/3。只压相同数量非证据chunk也都是3/3。
