...
def gpu():
 ...
 return {'foreign':foreign,'processes':raw.strip()}
...
@torch.inference_mode()
def gate(model,tok,example,eos,foil=None):
 release();gpu();a=Engine(model,{'kind':'native'},record=True);h=ShapeAudit(model)
 try:sa=a.prefill(a.new_state(),example['segments'])
 finally:h.close()
 result={'shape':shape(a,sa,h,example['segments']),'probes':{},'gates':[]}
 refs={p['name']:probe(a,sa,p['ids'],eos) for p in example['probes']}
 b=Engine(model,record=True,measure_depths=(18,27) if foil is not None else ());sb=b.prefill(b.new_state(),example['segments']);sb.validate();errs=[]
