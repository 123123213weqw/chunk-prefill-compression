...
class TestAuditHelpers(unittest.TestCase):
    ...
    def test_report_no_success_claim_from_audit_pass(self):
        scores={m:dict(readout_correct=2,control_correct=4,n=4,field_correct=8,fields=14,
                      pairs_both_correct=1,pairs=2,format_failures=0,token_caps=0,unavailable=0) for m in EXPECTED}
        a={'scores':scores,'sample_count':4,'pair_count':2,'length':4096,'identity_gates':16,'shape_audits':24,
           'linear_flop_ratios':{m:[.8] for m in CANDIDATES},'all_trial_boundaries_isolated':False,
           'timing_summary':[],'paired_comparisons':{},'stratified_scores':[],'comparisons':[]}
