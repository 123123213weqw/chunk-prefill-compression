...
class Engine(BaseEngine):
    def __init__(self,model,spec=None,thresholds=None,record=False,measure_depths=()):
        ...
        self.t_start=None
        self.chunk_keeps={int(k):int(v) for k,v in self.spec.get('chunk_keeps',{}).items()}
        if any(k<0 or v not in (16,32,64) for k,v in self.chunk_keeps.items()):raise ValueError('invalid diagnostic chunk override')
        if self.chunk_keeps and (self.kind!='fixed' or self.spec['keep'] not in (16,64)):raise ValueError('overrides require fixed keep16 or keep64')
            ...
            if stage=='T':
                ...
                if selected:
                    ...
                    for j,(part,p) in enumerate(zip(parts,pp)):
                        if j in selected:
                            target=self.chunk_keeps.get((start-self.t_start)//64+j,self.spec['keep']) if self.chunk_keeps else target
