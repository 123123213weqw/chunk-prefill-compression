...
def shrink(hidden,positions,method):
    """Query-blind 64→32 or 64→48 merge, anchored at group end positions.

    48: each consecutive quadruple [a,b,c,d] becomes [mean(a,b),c,d].
    The phase resets on 64-token chunk boundaries, never on question content.
    """
    ...
    elif method=='pair_mean_48':
        groups=hidden.reshape(hidden.shape[0],-1,4,hidden.shape[-1])
        merged=groups[:,:,:2,:].float().mean(2,keepdim=True).to(hidden.dtype)
        h=torch.cat((merged,groups[:,:,2:,:]),dim=2).reshape(hidden.shape[0],-1,hidden.shape[-1])
        return h,positions.reshape(-1,4)[:,1:].reshape(-1)
...
class Engine:
    def __init__(self,model,method='identity',split_depth=12,native=False):
        if method not in ('identity','pair_mean','pair_mean_48','endpoint'):raise ValueError(method)
