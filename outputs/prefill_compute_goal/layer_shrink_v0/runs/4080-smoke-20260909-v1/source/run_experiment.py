...
"""
import argparse,gc,hashlib,json,random,shutil,statistics,sys,time,traceback
...
METHODS=('native_full','layerwise_identity','pair_mean_d12','endpoint_d12')
...
def make_engine(model,name):
    return Engine(model,method='pair_mean' if name=='pair_mean_d12' else 'endpoint' if name=='endpoint_d12' else 'identity',
                  split_depth=12,native=name=='native_full')
...
def shape_result(audit,lengths,method):
    expected_full=sum(lengths.values());expected_upper=expected_full-lengths['T']//2 if method in ('pair_mean_d12','endpoint_d12') else expected_full
    counts=Counter()
    for r in audit.rows:counts[(r['layer'],r['op'])]+=r['tokens']
    for i in range(36):
        for op in ['q_proj','k_proj','v_proj','o_proj','gate_proj','up_proj','down_proj']:
            assert counts[(i,op)]==(expected_full if i<12 else expected_upper)
    return {'actual_module_inputs':audit.rows,'linear_flops':audit.linear_flops(),
            ...
            'linear_token_layer_ratio':(12*expected_full+24*expected_upper)/(36*expected_full)}
...
def correctness(model,tok,samples,args,out,event):
    ...
    rows=[];shapes=[];gates=[]
    # Compare native and identity BEFORE allowing lossy conditions.
    for s in samples:
        sid=s['source']['sample_id'];native=make_engine(model,'native_full');identity=make_engine(model,'layerwise_identity')
        a=native.prefill(native.new_state(),s['segments'],args.block)
        b=identity.prefill(identity.new_state(),s['segments'],args.block)
        ...
        for la,lb in zip(a.cache.layers,b.cache.layers):
            ...
            for x,y in [(la.keys,lb.keys),(la.values,lb.values)]:
                ...
                assert err<.005,('identity cache mismatch',sid,err)
                cache_errors.append(err)
        for p in s['probes']:
            ta,xa=probe(native,a,p,args.max_tokens,eos);tb,xb=probe(identity,b,p,args.max_tokens,eos)
            ...
            gate={'sample_id':sid,'probe':p['probe_id'],'max_cache_relative_rms':max(cache_errors),'logit_relative_rms':logit_error,
        ...
        del a,b;release()
        event({'event':'identity_gate_pass','sample_id':sid})
    for s in samples:
        ...
        for method in METHODS:
            ...
            for p in s['probes']:
                ...
                row={'sample_id':sid,'pair_id':s['source']['pair_id'],'template':s['source']['template'],'method':method,'probe':p['probe_id'],
                     'answer':answer,'gold':p['gold'],'generated_tokens':len(ids),'hit_token_cap':len(ids)==args.max_tokens,
...
def benchmark(model,samples,args,out,event):
    methods=['native_b1024','native_b4096','layerwise_identity','pair_mean_d12','endpoint_d12']
    rows=[]
...
def main():
    ...
    if args.block!=1024:raise ValueError('v0 preregistered block=1024; version protocol before changing')
    if args.warmups<1 or args.repeats<5:raise ValueError('require >=1 warmup and >=5 repeats')
    ...
    out=Path(args.out)
    if (out/'manifest.json').exists():raise FileExistsError('immutable run directory already used')
    ...
    if len(raw)!=4 or len({s['pair_id'] for s in raw})!=2 or any(s['split']!='development' or s['length']!=4096 for s in raw):
        raise ValueError('v0 requires frozen 4K development smoke4; no held-out evaluation')
    for file in [Path(__file__),ROOT/'engine.py',ROOT/'test_engine.py',ROOT.parents[1]/'validation_v3/methods.py',Path(args.dataset)]:shutil.copy2(file,out/'source'/file.name)
    meta={'status':'running','args':vars(args),'scope':'hidden-state layer shrinking, untrained development pilot',
